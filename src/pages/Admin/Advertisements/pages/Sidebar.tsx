...
const SidebarAdvertisementsPage: React.FC = () => {
  ...
  const fetchAdvertisements = async () => {
    ...
    try {
      ...
      // Filter sidebar advertisements
      setAdvertisements(data.filter(ad => ad.placement === 'sidebar'));
    } catch (error) {
      console.error('Failed to fetch sidebar advertisements:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAddClick = () => {
    // Pre-select sidebar placement when adding new advertisement
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">{t('advertisements.sidebarPanel')}</h1>
        <button
          onClick={handleAddClick}
          className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
        >
          <Plus className="mr-2 h-4 w-4" />
          {t('advertisements.addSidebar')}
        </button>
      </div>
...
export default SidebarAdvertisementsPage;
